...
public class ReportServiceImpl implements ReportService {
    ...
    public void generateOrdersReport(String dateStart, String dateEnd) {
        try {
            float smallRowGap = 5f;
            float bigRowGap = 40f;
            float middleRowGap = 20f;

            LocalDate dStart = dateStart == null || dateStart.isEmpty() ? convertToLocalDateViaInstant(orderRepository.getMinDate()) : LocalDate.parse(dateStart);
            LocalDate dEnd = dateEnd == null || dateEnd.isEmpty() ? convertToLocalDateViaInstant(orderRepository.getMaxDate()): LocalDate.parse(dateEnd);

            dateStart = formatDate(convertToDateViaSqlTimestamp(dStart));
            dateEnd = formatDate(convertToDateViaSqlTimestamp(dEnd));
            ...
            addTableHeader(Stream.of("Номер замовлення","Дата","Статус", "К-сть найменувань товарів", "Загальна к-сть одиниць", "К-сть категорій товарів","К-сть постачальників", "Середня ціна на одинию товару (грн)", "Вартість замовлення (грн)"),
                    table1, headerFont);
    ...
    private void addRows1(PdfPTable table, Iterable<OrderReportValues> productReportValues, Font font) {
        for (OrderReportValues reportValues: productReportValues) {
